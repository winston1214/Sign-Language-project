...
def check_input():
    ...
    # 우리가 사용할 함수
    # return 값이 mode인 'video'와 videofile(args.video)
    # for video
    if len(args.video):
...
def alphapose_inference(checkpoint,cfg_path,format,outdir,sp = True):
    ...
    # MODEL : FastPose (pretrained model?)
    ...
    if args.save_video and mode != 'image':
        # DEFAULT_VIDEO_SAVE_OPT : cv2.VideoWriter_fourcc(*'mp4v')
        ...
        # update가 중요! json으로 저장하는 부분-> 형태 알아보기 쉽게 수정하기form == 'open':
